...
T = zeros(2000, 100); % Initialize matrix to store MSE values for each iteration
f = 1000;
fsamp = 10000;
N = 2000;
mu = 0.1;
...
% Store MSE for each iteration in T matrix
for k = 1:100
    w = zeros(L, 1);
    %e = zeros(1, N);
    MSE = zeros(1, N);
...
% Apply a moving average filter to smooth out the MSE curve
windowSize = 20;
b = (1/windowSize)*ones(1,windowSize);
a = 1;
mse_avg_smoothed = filter(b, a, mse_avg);

figure;
plot(10 * log10(mse_avg_smoothed));
title('Smoothed Average MSE in dB');
xlabel('Iteration');
ylabel('MSE (dB)');
